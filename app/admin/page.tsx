...
export default function AdminDashboard() {
  ...
  // Enhanced download function that works with both file paths and URLs
  const handleDownload = async (photo: Photo) => {
    try {
      ...
      if (photo.file_path) {
        // If we have a file_path, use Supabase storage
        const { data } = supabase.storage
          .from("Uploads")
          .getPublicUrl(photo.file_path, { download: true });
        
        downloadUrl = data.publicUrl;
        fileName = photo.file_path.split("/").pop() || `photo_${photo.id}.jpg`;
      } else if (photo.src) {
        // Fallback to direct URL download
        downloadUrl = photo.src;
        fileName = `photo_${photo.id || Date.now()}.jpg`;
      } else {
